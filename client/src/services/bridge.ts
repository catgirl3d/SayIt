...
/**
 * Returns the system display language as an `en` / `uk` frontend locale.
 *
 * This differs from `getClientRuntimeInfo().systemLocale`, which is a regional
 * formatting value used by diagnostics. The Rust resolver is shared by the tray
 * and the main interface so they cannot make different decisions.
 */
